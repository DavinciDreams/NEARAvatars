...
export default function ConnectMint() {
  ...
  //console.log("import.meta.env.VITE_APP_USE_POLKADOT is", import.meta.env.VITE_APP_USE_POLKADOT)
  //const { polkadot }: any = window;
  ...
  useEffect(() => { 
    if (glb && screenshot) {
      if(import.meta.env.VITE_APP_USE_IC){
        mintAvatarToIC();
      //if(import.meta.env.VITE_APP_USE_POLKADOT){
        //mintAvatarToPolkadot();
      } else {
...
/* 
    const mintAvatarToPolkadot = async () => {
      //////////////////////////// upload part //////////////////////
      /// ---------- glb -------------- ////////////////
      const formData = new FormData();
      formData.append("profile", glb);
...
/*      // Polkadot
     if(account){
      const signer = new ethers.providers.Web3Provider(polkadot).getSigner();
      const contract = new ethers.Contract(contractAddress, contractABI, signer);
      const MintedToken = await contract.totalSupply();
      setTotalMinted(parseInt(MintedToken));
    }
...
{/*               {import.meta.env.VITE_APP_USE_POLKADOT === "true" && !connected &&
          <Button
            variant="contained"
            startIcon={<AccountBalanceWalletIcon />}
            onClick={connectWallet}
